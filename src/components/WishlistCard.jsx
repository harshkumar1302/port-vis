...
const WishlistCard = ({ art }) => {
  ...
  const handleRemove = (e) => {
    ...
    e.stopPropagation();
    setIsRemoving(true);
    // Short delay so they see the heart unfill before it vanishes
    setTimeout(() => {
      toggleWishlist(art);
    }, 300);
  };
  ...
  return (
    <article className="group cursor-pointer flex flex-col items-center relative">
        <div className={`w-full bg-white rounded-3xl overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] transition-all duration-500 mb-4 border border-ghibli-wood/5 relative ${isRemoving ? 'opacity-50 scale-95' : ''}`}>
            ...
            {/* Wishlist specific controls overlay */}
            <div className="absolute top-4 right-4 z-20 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
                ...
                >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={isRemoving ? "none" : "currentColor"} stroke="currentColor" strokeWidth={isRemoving ? "2" : "0"} className="w-5 h-5 transition-all duration-300">
        ...
        <div className="text-center px-2 opacity-90 group-hover:opacity-100 transition-opacity w-full">
            ...
            <div className="flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest font-bold">
                <span className="text-ghibli-charcoal/40">
