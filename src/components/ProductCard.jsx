...
const ProductCard = ({ art, priority = false, linkTo, linkState, variant = 'default' }) => {
  const { toggleWishlist, isInWishlist, addToCart, moveToCart } = useStore();
  ...
  return (
    <div className="group bg-white rounded-2xl sm:rounded-[32px] overflow-hidden shadow-soft hover:shadow-[0_20px_50px_rgba(0,0,0,0.12)] transition-all duration-500 flex flex-col h-full relative translate-y-0 hover:-translate-y-2">
      ...
      <div className="p-3 sm:p-5 flex flex-col flex-grow bg-white/50 backdrop-blur-sm">
        ...
        <div className={`flex items-end justify-between mt-auto pt-2 ${isWishlistView ? 'flex-col items-stretch gap-3' : ''}`}>
          ...
          {isWishlistView ? (
            isOutOfStock ? (
              <button
                type="button"
                disabled
                className="w-full py-3 px-4 rounded-xl border border-ghibli-charcoal/20 text-sm font-bold text-ghibli-charcoal/40 bg-ghibli-paper cursor-not-allowed"
              >
                Out of stock
              </button>
            ) : (
              <button
                ...
                className="w-full py-3 px-4 rounded-xl bg-ghibli-wood text-white text-sm font-bold hover:bg-ghibli-navy transition-colors active:scale-[0.98]"
              >
                Move to cart
              </button>
            )
